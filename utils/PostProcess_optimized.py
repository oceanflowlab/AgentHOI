...
class PostProcess(object):
    ...
    def batch_call(self, batch_outputs, hoi_mapper, img_ids):
        """批量处理版本，一次处理多个样本"""
        batch_results = {}
        
        batch_size = batch_outputs['pred_logits'].shape[0]
        
        for i in range(batch_size):
            single_outputs = {
                'pred_logits': batch_outputs['pred_logits'][i:i+1],
                'pred_boxes': batch_outputs['pred_boxes'][i:i+1], 
                'box_scores': batch_outputs['box_scores'][i:i+1]
            }
            
            result = self.__call__(single_outputs, hoi_mapper)
            batch_results[img_ids[i]] = result
