# 第2步，映射Object Category和Interaction Class
...
from hico_categories import HICO_OBJECTS
...
class HICOMapper:
    def __init__(self):
        ...
        # 保存HICO对象和交互信息
        self.hico_objects = HICO_OBJECTS
        # 创建对象名称列表和对象到ID的映射
        self.object_names = [obj["name"] for obj in self.hico_objects]
...
def process_results(input_file: str) -> List[Dict]:
    ...
    with open('data/hico/test_hico_ann.json', 'r', encoding='utf-8') as f:
        gt = json.load(f)

    gt_file2size = {item['file_name']: [item['height'], item['width']] for item in gt}
    gt_file = [item['file_name'] for item in gt]
    ...
    for result in tqdm(results):
        if result["filename"] not in gt_file:
        ...
        processed_item = {
            ...
            "Human Description": [],
            "Object Description":[],
            "orig_size": gt_file2size[result["filename"]],
        }
