...
from hico_categories import HICO_OBJECTS, HICO_INTERACTIONS # 保留您的导入
...
# --- 核心逻辑与数据处理 ---
def build_object_to_actions_map(hico_interactions: List[Dict[str, str]]) -> defaultdict:
    """构建对象到其可能动作列表的映射。"""
...
def extract_prompt_info_for_item(
    ...
    for i in range(num_objects):
        ...
        possible_actions_for_oc = [
            ...
            # 您之前的代码注释掉了 if action != "no_interaction"，我将保持这个逻辑
            # 如果需要过滤 "no_interaction"，请取消下面这行的注释
...
async def process_all_data_async(args: argparse.Namespace):
    ...
    print("初始化资源...")
    hico_object_to_actions = build_object_to_actions_map(HICO_INTERACTIONS)
...
if __name__ == "__main__":
    if not HICO_INTERACTIONS: # 确保 HICO_INTERACTIONS 有效
        print("错误: HICO_INTERACTIONS 未定义或为空。请确保从 'hico_categories' 正确导入或提供数据。")
        # exit(1) # 如果这是致命错误，可以选择退出
    main_cli()