...
import json
import numpy as np
from .swig_v1_categories import SWIG_INTERACTIONS, SWIG_CATEGORIES, SWIG_ACTIONS


class SWiGEvaluator(object):
    ''' Evaluator for SWIG-HOI datasets '''
    def __init__(self, anno_file, output_dir):
        ...
        self.id2obj = {obj["id"]: obj["name"] for obj in SWIG_CATEGORIES}
        self.id2act = {act["id"]: act["name"] for act in SWIG_ACTIONS}
        self.hoi_mapper = {interaction["id"]: self.id2act[interaction["action_id"]] + " " + self.id2obj[interaction["object_id"]]
...
def calc_ap(scores, boxes, keys, gt_boxes):
    ...
    for i in range(min(len(idx), 69999)):
        pair_id = idx[i]
        box = boxes[pair_id, :]
        key = keys[pair_id]
        if key in gt_boxes:
...
def prepare_swig_gts(anno_file):
    """
    Convert datasets to the format required by evaluator.
    """
    with open(anno_file, "r") as f:
        dataset_dicts = json.load(f)
